...
func TestArchivabilityDrill038(t *testing.T) {
	dsn := os.Getenv("TEEPIN_DRILL_DSN")
	if dsn == "" {
		t.Skip("set TEEPIN_DRILL_DSN to run the migration drill")
	}
	...
	if !tableExists(t, db, "compute", "instance_metrics") {
		t.Fatal("after up: compute.instance_metrics missing")
	}

	if err := migrator(t, db).Migrate(37); err != nil {
		t.Fatalf("migrate down to 037: %v", err)
	}
	if tableExists(t, db, "compute", "instance_metrics") {
		t.Error("after down: compute.instance_metrics still present")
	}
	if !tableExists(t, db, "compute", "instances") {
		t.Error("after down: compute.instances was wrongly removed")
	}
	...
	t.Log("archivability drill passed: 038 applies, reverts cleanly, and re-applies")
}

func TestArchivabilityDrill039(t *testing.T) {
	dsn := os.Getenv("TEEPIN_DRILL_DSN")
	if dsn == "" {
		t.Skip("set TEEPIN_DRILL_DSN to run the migration drill")
	}
	...
	if !columnExists(t, db, "auth", "projects", "allow_on_demand") {
		t.Fatal("after up: auth.projects.allow_on_demand missing")
	}

	if err := migrator(t, db).Migrate(38); err != nil {
		t.Fatalf("migrate down to 038: %v", err)
	}
	if columnExists(t, db, "auth", "projects", "allow_on_demand") {
		t.Error("after down: auth.projects.allow_on_demand still present")
	}
	if !tableExists(t, db, "auth", "projects") {
		t.Error("after down: auth.projects was wrongly removed")
	}
...
func TestArchivabilityDrill042(t *testing.T) {
	dsn := os.Getenv("TEEPIN_DRILL_DSN")
	if dsn == "" {
		t.Skip("set TEEPIN_DRILL_DSN to run the migration drill")
	}
	...
	if tableExists(t, db, "storage", "backend_health") {
		t.Fatal("after up: storage.backend_health should be gone (042 drops what 041 created)")
	}

	if err := migrator(t, db).Migrate(41); err != nil {
		t.Fatalf("migrate down to 041: %v", err)
	}
	if !tableExists(t, db, "storage", "backend_health") {
		t.Error("after down to 041: storage.backend_health should be back")
	}

	if err := migrator(t, db).Up(); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("re-up: %v", err)
	}
	t.Log("archivability drill passed: 042 applies, reverts cleanly, and re-applies")
}

func TestArchivabilityDrill043(t *testing.T) {
	dsn := os.Getenv("TEEPIN_DRILL_DSN")
	if dsn == "" {
		t.Skip("set TEEPIN_DRILL_DSN to run the migration drill")
	}
	...
	if !columnExists(t, db, "billing", "pricing", "object_storage_price_per_gb_month") {
		t.Fatal("after up: billing.pricing.object_storage_price_per_gb_month missing")
	}
	if !columnExists(t, db, "billing", "pricing", "object_storage_price_per_gb_egress") {
		t.Fatal("after up: billing.pricing.object_storage_price_per_gb_egress missing")
	}

	if err := migrator(t, db).Migrate(42); err != nil {
		t.Fatalf("migrate down to 042: %v", err)
	}
	if columnExists(t, db, "billing", "pricing", "object_storage_price_per_gb_month") {
		t.Error("after down: object_storage_price_per_gb_month still present")
	}
	if !tableExists(t, db, "billing", "pricing") {
		t.Error("after down: billing.pricing was wrongly removed")
	}

	if err := migrator(t, db).Up(); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("re-up: %v", err)
	}
	t.Log("archivability drill passed: 043 applies, reverts cleanly, and re-applies")
}
