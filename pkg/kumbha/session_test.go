...
func TestStore_Create_RejectsNonPositiveBudget(t *testing.T) {
	...
	_, err := store.Create(context.Background(), uuid.New(), uuid.New(), 0, "test", "teepin/qwen3-30b-a3b")
	if err == nil {
...
func TestStore_Create_RejectsOverCapBudget(t *testing.T) {
	store, _ := newMockStore(t)
	_, err := store.Create(context.Background(), uuid.New(), uuid.New(), maxSessionBudget+0.01, "test", "teepin/qwen3-30b-a3b")
	if err == nil {
...
func TestStore_Create_InsertsAndReturnsSession(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO billing\.inference_sessions`).
		WithArgs(accountID, projectID, 5.0, "build booking app", "teepin/qwen3-30b-a3b").
		WillReturnRows(sqlmock.NewRows([]string{"id", "spent", "status", "started_at"}).
			AddRow(sessionID, 0.0, "open", now))

	sess, err := store.Create(context.Background(), accountID, projectID, 5.0, "build booking app", "teepin/qwen3-30b-a3b")
	if err != nil {
