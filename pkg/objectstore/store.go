...
// InsertProbeResult records one health-probe reading (see probe.go). Never
// returns a soft failure to the prober's caller as fatal — a probe whose
// own bookkeeping write fails is logged and the probe loop just continues,
// since losing one history row is never worth stopping monitoring over.
func (s *Store) InsertProbeResult(ctx context.Context, r ProbeResult) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO storage.backend_health (backend, probe, ok, latency_ms, error, checked_at)
		VALUES ($1, $2, $3, $4, $5, $6)
	`, r.Backend, r.Probe, r.OK, r.LatencyMS, nullIfEmpty(r.Error), r.CheckedAt)
	if err != nil {
		return fmt.Errorf("objectstore: insert probe result: %w", err)
	}
	return nil
}

// RecentProbeResults returns a backend's most recent probe readings,
// newest first — the sole input to health-status derivation (see
// probe.go's DeriveHealthStatus) and to whatever renders a latency
// history in the console's storage service tab.
func (s *Store) RecentProbeResults(ctx context.Context, backend string, limit int) ([]ProbeResult, error) {
	if limit <= 0 || limit > 500 {
		limit = 500
	}
	rows, err := s.db.QueryContext(ctx, `
		SELECT backend, probe, ok, latency_ms, COALESCE(error, ''), checked_at
		FROM storage.backend_health
		WHERE backend = $1
		ORDER BY checked_at DESC
		LIMIT $2
	`, backend, limit)
	if err != nil {
		return nil, fmt.Errorf("objectstore: query recent probe results: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r ProbeResult
		if err := rows.Scan(&r.Backend, &r.Probe, &r.OK, &r.LatencyMS, &r.Error, &r.CheckedAt); err != nil {
			return nil, fmt.Errorf("objectstore: scan probe result: %w", err)
		}
		out = append(out, r)
	}
	return out, rows.Err()
